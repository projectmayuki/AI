...
class IServerProcess:
    ...
    def __init__(self, port, opt = {"ServerMaxClientNum" : 5, "ServerBufferSize" : 4096}):
        self._port = port
        self._opt = opt
    # __init__
    ...
    def execute(self):
        with socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as _socket:
            _socket.bind( (IServerProcess.ADDR, self._port) )

            while True:
                recieve_raw_data, client_addr = _socket.recvfrom(self._opt["ServerBufferSize"])
